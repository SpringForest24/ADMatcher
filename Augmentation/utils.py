...
def mask_nodes(data, pn):
    ...
    data.x[idx_mask] = torch.tensor(np.random.normal(loc=0.5, scale=0.5, size=(mask_num, feat_dim)), dtype=torch.float32, device=device)#使用正态分布来替换
...
class Synthetic_Dataset(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
        super(Synthetic_Dataset, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def process(self):
        data_list = []
        for _ in range(1000):
            n_nodes = torch.randint(10, 50, (1,)).item()
            p_edge = torch.rand(1).item() * 0.3 + 0.1
            g = nx.erdos_renyi_graph(n_nodes, p_edge)
            edge_index = torch.tensor(list(g.edges)).t().contiguous()
            # x = torch.tensor([[1]] * n_nodes, dtype=torch.float)#全1特征
            x = torch.randn(n_nodes, 10) # 生成 10 维的高斯分布特征
